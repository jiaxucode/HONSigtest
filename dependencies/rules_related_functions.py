...
def DumpSignificantRules(statisticsindex_of_different_dependecies, zscore_outliers, Rules, SaveRulesFolder, OutputRulesFile):
    '''
    ...
    for source in zscore_outliers:
        for target in zscore_outliers[source]:
            ...
            # if (z_score > 1.96) | (z_score < -1.96):  # 把95%CI显著的高阶依赖模式保存
